...
def pretty_print_group(key, aliases, highlight=None, only_groupname=False):
    if len(aliases) == 0:
        return
    group_hl_formatter = lambda g, hl: termcolor.colored(hl, 'green', None, ['bold']).join([termcolor.colored(part, 'green', None, ['bold']) for part in ('*  %s' % g).split(hl)])
    alias_hl_formatter = lambda alias, hl: termcolor.colored(hl, None).join([termcolor.colored(part, None, None, ['dark']) for part in ('   %s = %s' % alias[0:2]).split(hl)])
    group_formatter = lambda g: termcolor.colored('*  %s' % g, 'green', None, ['bold'])
    alias_formatter = lambda alias: termcolor.colored('   %s = %s' % alias[0:2], None)
